...
export default function SubjectListPage() {
  ...
  useEffect(() => {
    ...
    fetch("http://127.0.0.1:8000/api/subject-offerings/", {
      ...
      // supports either array or paginated {results: []}
  ...
  if (offerings.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-4">
        ...
        <Link to="/teacher/subject/create-subject" className="bg-indigo-600 text-white px-6 py-2 rounded-lg">
  ...
  return (
    <section className="bg-slate-50 min-h-screen p-4">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          <h1 className="text-3xl font-black text-slate-900">Subject Offerings</h1>
      ...
      <div className="mb-6 text-slate-600">
        <strong>{totalClasses}</strong> offerings • <strong>{totalStudents}</strong> students
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {offerings.map((o) => (
          <Link
            ...
            className="bg-white border border-slate-200 rounded-2xl p-4 hover:shadow-xl transition relative group"
          >
            {/* Top Row */}
            ...
            <div className="bg-slate-50 rounded-xl p-3 mb-3 grid grid-cols-2 gap-2">
              <div>
                ...
                <p className="font-semibold text-slate-700">{o.room_number}</p>
              </div>
              <div>
                <span className="text-xs text-slate-400 uppercase">Schedule</span>
                <p className="font-semibold text-slate-700">{o.schedule}</p>
              </div>
            </div>
            ...
            {deletingId === o.id && <div className="mt-3 text-xs text-slate-500">Deleting...</div>}
          </Link>
        ))}
      </div>
    </section>
