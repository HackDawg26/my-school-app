...
const AccountListPage = () => {
    ...
    const admins: UserAccount[] = [
        { id: "1001", name: "Admin User", email: "admin@claroed.com", status: "Active" },
    ];

    const currentList = activeTab === "teacher" ? teachers : activeTab === "admin" ? admins : students;
...
const accountrole = (isOpen : boolean) => {
            ...
            {isOpen && (
                <div className="absolute right-5 top-16 mt-2 w-48 bg-white rounded-md shadow-lg border border-gray-200 z-50">
                    <div className="py-1">
                        ...
                        <button onClick={() => navigate("/admin/accounts/create/admin")} className="flex items-center w-full px-4 py-2 text-sm text-red-600 hover:bg-red-50">
                            Admin
                        </button>
                    </div>
                </div>
